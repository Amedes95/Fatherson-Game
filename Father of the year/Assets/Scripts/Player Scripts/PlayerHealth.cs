...
public class PlayerHealth : MonoBehaviour
{
    ...
    private void Update()
    {
        DeathCount = PlayerData.LifetimeDeaths;
    }
    ...
    public void KillPlayer() // Kills player
    {
        ...
        PlayerData.LifetimeDeaths = PlayerData.LifetimeDeaths += 1; // update lifetime deaths
        ...
        if (PlayerData.AchievementRecords.ContainsKey("Let's try that again") == false && DeathCount >= 0) // not unlocked already?
        {
            PlayerData.AchievementRecords.Add("Let's try that again", 1); // add to unlock dictionary
        ...
        if (PlayerData.AchievementRecords.ContainsKey("20th time's the charm") == false && DeathCount >= 19) // not unlocked already?
        {
            PlayerData.AchievementRecords.Add("20th time's the charm", 1); // add to unlock dictionary
            Debug.Log("20th time's the charm");
            BackgroundMusic BGMusic = GameObject.FindGameObjectWithTag("BGMusic").GetComponent<BackgroundMusic>();
            BGMusic.UnlockCheevo("20th time's the charm");
        }
        /// die 200 times achievement
        if (PlayerData.AchievementRecords.ContainsKey("Lucky 200") == false && DeathCount >= 199) // not unlocked already?
        {
            PlayerData.AchievementRecords.Add("Lucky 200", 1);
            Debug.Log("Lucky 200");
            BackgroundMusic BGMusic = GameObject.FindGameObjectWithTag("BGMusic").GetComponent<BackgroundMusic>();
